...
def next_event_from_url(ical_url: str) -> Iterator[Event]:
    ...
    if response.status_code != 200:
        log.error('Failed to fetch calendar data from %s due to %s %s', ical_url, response.status_code, response.text)
        return iter([])
    ...
    for component in cal.walk(name="VEVENT"):
        events.append(Event(
            ...
            calendar_name=str(cal['X-WR-CALNAME']) if 'X-WR-CALNAME' in cal else 'Unnamed Calendar'
        ))

    # Filter future events and sort them by start time
    now = datetime.now().astimezone()
    return (event for event in events if
            now < event.start < now + timedelta(days=1))
...
class CalendarService(Service):
    ...
    @property
    def next_event(self):
        return next((e for e in self.__next_events if e.start > datetime.now().astimezone()), None)

    def update(self):
        log.info('Updating calendar data')
        # Update the calendar data from  the configured iCal feeds
        self.__next_events = sorted((
            event for calendar in self.calendars for event in next_event_from_url(calendar['url'])
